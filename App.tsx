  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case AppView.PREMIUM:
        return <SubscriptionView isPremium={isPremium} onSubscribe={handleUnlock} currency={settings.currency} region={settings.region} onOpenLegal={(type: 'privacy' | 'terms') => { setLegalType(type); handleNavigate(AppView.LEGAL); }} />;
      ...
      <div className="flex flex-col h-[100dvh] bg-calm-bg font-sans overflow-hidden text-gray-900">
        ...
        <div className="flex flex-1 overflow-hidden h-full">
          <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 z-20 h-full">
              <Navigation currentView={currentView} setView={(v: AppView) => { if (v === AppView.SETTINGS) setIsSettingsOpen(true); else handleNavigate(v); }} isPremium={isPremium} />
          </aside>

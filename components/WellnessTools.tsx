...
const WellnessTools: React.FC<WellnessToolsProps> = ({ isPremium, onUnlock }) => {
  ...
  return (
    <div className="p-4 space-y-8 max-w-3xl mx-auto pb-24">
      ...
      <div>
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 ml-1">Daily Essentials</h3>
      ...
      {/* SECTION 2: RELAXATION & PLAY (Premium Highlight) */}
      <div>
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 ml-1">Relaxation & Play</h3>
        <div className="grid gap-4 md:grid-cols-2">
            {/* Stress Relief Games */}
            <div 
              ...
              className={`p-6 rounded-2xl border transition-all relative overflow-hidden group 
                ${!isPremium 
                  ? 'bg-gradient-to-br from-gray-50 to-white border-dashed border-gray-300 cursor-pointer hover:border-pink-300' 
                  : 'bg-white border-gray-100 shadow-sm hover:shadow-md cursor-pointer hover:border-pink-200'}`}
            >
              {!isPremium && (
              ...
              <h3 className={`text-lg font-bold mb-1 ${!isPremium ? 'text-gray-500' : 'text-gray-800'}`}>Stress Relief Games</h3>
              <p className={`text-sm mb-4 ${!isPremium ? 'text-gray-400' : 'text-gray-500'}`}>
                Pop bubbles or break balloons. Simple interactive distractions to calm your mind.
              </p>
            ...
            <div 
              ...
              className={`p-6 rounded-2xl border transition-all relative overflow-hidden group 
                ${!isPremium 
                  ? 'bg-gradient-to-br from-gray-50 to-white border-dashed border-gray-300 cursor-pointer hover:border-indigo-300' 
                  : 'bg-white border-gray-100 shadow-sm hover:shadow-md cursor-pointer hover:border-indigo-200'}`}
            >
              {!isPremium && (
                <div className="absolute top-3 right-3">
                  <i className="fa-solid fa-lock text-gray-400"></i>
              ...
              <h3 className={`text-lg font-bold mb-1 ${!isPremium ? 'text-gray-500' : 'text-gray-800'}`}>Sleep Sounds</h3>
              <p className={`text-sm mb-4 ${!isPremium ? 'text-gray-400' : 'text-gray-500'}`}>

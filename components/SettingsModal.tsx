    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-[70] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden animate-fade-in">
        ...
        <div className="p-6 space-y-6">
          {/* Region Selection */}
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">My Region</label>
            <div className="relative">
              <select 
                value={settings.region} 
                onChange={handleRegionChange}
                className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl appearance-none focus:outline-none focus:ring-2 focus:ring-brand-500"
              >
                {REGIONS.map(region => (
          ...
          {/* Language Selection */}
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">AI Language</label>
            <div className="relative">
              <select 
          ...
          <button 
            onClick={onClose}
            className="w-full bg-brand-600 text-white font-bold py-3 rounded-xl shadow-md hover:bg-brand-700 transition-colors"
          >
            Save Changes
          </button>
        </div>
